...
net.xirvik.seedbox = (function(my) {
    my.extension = {
        ...
        onContextMenu: async function(e) {
            // must complete before reading options: the service worker may have
            // just restarted and the in-memory options are still the defaults
            ...
            if(e.menuItemId === 'enable') {
                ...
                for(var i = 0; i < my.extension.options.servers.length; i++) {
                    if(my.extension.menuId(my.extension.options.servers[i]) == e.menuItemId) {
        ...
        createContextMenuItem: function(index, root) {
            var name = this.menuId(this.options.servers[index]);
            var options = {
                id: name,
                title: name,
                contexts: ['link'],
